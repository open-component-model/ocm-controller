...
kind create cluster --name=e2e-test-ocm-cluster
...
kubectl wait --for=condition=Ready=true Certificate/ocm-registry-certificate -n ocm-system --timeout=60s
echo 'done'

kubectl get secret ocm-registry-tls-certs -n ocm-system -o jsonpath="{.data['tls\.crt']}" | base64 -d > hack/rootCA.pem
